...
impl<H: Helper> Refresher for State<'_, '_, H> {
    ...
    fn external_print(&mut self, msg: String) -> Result<()> {
        // WAASH patch (flicker fix): the stock implementation cleared the
        // prompt rows in a separate flush *before* redrawing, so the prompt
        // visibly blanked out on every refresh. Instead we let refresh_line()
        // clear the old rows and redraw in a single atomic write (the same
        // flicker-free path normal keystrokes use). The message is still
        // written; for WAASH's live-refresh trigger it is a bare "\x1b[1A"
        // (cursor up), which combined with the forced newline below nets to
        // zero cursor movement, so only the atomic redraw is visible.
        //
        // WAASH patch (job-control safety): if the user is actively editing a
        // non-empty line, skip the refresh entirely. The live-refresh ticker
        // fires asynchronously (its message travels through a pipe and is
        // processed by this main-thread read loop), so a tick started while
        // idle can land right as the user types/Enter. Refresh_line() would
        // re-lay-out the row and consume the in-flight Enter, swallowing the
        // submitted command. An empty line means there is nothing typed to
        // protect, so a repaint is safe; a non-empty line means the user is
        // typing, so we must not disturb it.
